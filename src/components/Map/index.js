  ...
  const encoded =
    ...
    window.btoa(`
      <svg style="color: white;" width="40" height="40" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="-7 -7 14 14">
        <circle r="5.5" stroke="${active ? 'red' : 'transparent'}" strokeWidth="2" fill="transparent" />
...
const Map = withScriptjs(withGoogleMap(({racks, activeId, position, onDragEnd}) => {
  const refs = {
    map: undefined
  }
  return (
    ...
    >
      {racks.map(rack => (
        <Marker
          key={rack.id}
          labelColor="white"
          label={{text: rack.bikesAvail.toString(), color: 'white', fontFamily: 'IBM Plex Mono', fontSize: '12px'}}
